...
# ---------- 5. JaKooLit Hyprland (24.04 Branch) ----------
echo "🌊 Step 5/12: Installing Hyprland via JaKooLit (24.04)..."
cd $HOME
if [ ! -d "Ubuntu-Hyprland" ]; then
    git clone -b 24.04 --depth=1 https://github.com/JaKooLit/Ubuntu-Hyprland.git
fi
cd Ubuntu-Hyprland
chmod +x install.sh

# Skip SDDM since we use GDM (no fingerprint support needed)
./install.sh --skip-sddm

# Verify Hyprland installation
if ! command -v Hyprland &> /dev/null; then
    echo "⚠️  WARNING: Hyprland installation may have failed!"
    echo "Check ~/Ubuntu-Hyprland directory and install.sh output"
fi

# ---------- 6. Register Hyprland Session for GDM ----------
echo "🔐 Step 6/12: Registering Hyprland Wayland Session..."
...
# ---------- 7. Heavy Workload Performance Tuning ----------
echo "⚡ Step 7/12: Heavy Workload Performance Optimizations..."
...
# ---------- 8. TLP Power Management Configuration ----------
echo "🔋 Step 8/12: Configuring TLP for Performance..."
...
# ---------- 9. Timeshift (Btrfs Snapshot System) ----------
echo "📸 Step 9/12: Installing Timeshift for Btrfs snapshots..."
...
echo "🛠️  Step 10/12: Installing Essential Utilities..."
...
# ---------- 11. App Management (Flatpak + bauh) ----------
echo "📦 Step 11/12: Setting up Flatpak & bauh app store..."
...
# ---------- 12. CUPS Printing (disabled by default) ----------
echo "🖨️  Step 12/12: Installing CUPS (disabled by default)..."
